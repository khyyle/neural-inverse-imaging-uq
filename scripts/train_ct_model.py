...
def build_training_objects(
    problem_config: RadonProblemConfig,
    model_config: FourierFeatureModelConfig,
) -> tuple[FourierFeatureMLP, jax.Array, LinearInverseProblem]:
...
def build_model_name(config: RadonProblemConfig) -> str:
    ...
    return f"ct/{source_name}/{config.pixel_count}x{config.pixel_count}/{MODEL_LABEL}"
...
def train(
    ...
    request = ModalTrainingRequest.from_configs(
        builder_path="train_ct_model:build_training_objects",
