...
def build_training_objects(
    problem_config: VlbiProblemConfig,
    model_config: NeuralImageConfig,
...
def build_model_name(config: VlbiProblemConfig) -> str:
    ...
    return (
        f"vlbi/{source_name}/{array_name}/{config.pixel_count}x{config.pixel_count}/{MODEL_LABEL}"
    )
...
def train(
    ...
    request = ModalTrainingRequest.from_configs(
        builder_path="train_vlbi_model:build_training_objects",
