"""Evaluate CT uncertainty for a caller-selected image model."""
...
def _save_figures(
    ...
    """Build and save the standard CT evaluation figures."""
...
def run(config: ExperimentConfig) -> Path:
    """Evaluate and persist one CT uncertainty result."""
    saved_model = open_saved_model(config.saved_model_path)
    problem_config = RadonProblemConfig.from_dict(
        saved_model.problem_metadata
    )
...
def main() -> None:
    """Run one tracked CT uncertainty evaluation."""
